...
export function AportesPageClient() {
  const router = useRouter();
  const { user } = useAuth();
  const { enteAtual } = useEnte();
  const [aportes, setAportes] = useState<Aporte[]>([]);
  const { data: entes = [] } = useEntesQuery();
  ...
  const { data: aportesData = [], isLoading } = useAportesQuery({ enteId: undefined, ano: undefined });
                            ...
                            {expandedAnos.has(anoKey) && (
                              <div className="p-4 space-y-4">
                                {anoGroup.entes.map((enteData) => (
                                  <div
                                    key={enteData.ente.id}
                                    className="border-l-4 border-blue-400 dark:border-blue-600 pl-4"
                                  >
